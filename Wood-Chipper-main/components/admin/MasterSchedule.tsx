            ...
            {localEmployees.map((emp, idx) => {
              ...
              return (
                <tr key={emp.id} className="border-b border-white/5 hover:bg-cyan-500/5 transition-colors group text-[10px]">
                  <td className="p-2 border border-slate-700/30 text-center font-bold text-slate-500">{idx + 1}</td>
                    ...
                    return (
                      <td key={day} className={`p-0 border border-slate-700/30 min-w-[30px] ${isRest ? 'bg-red-900/40' : ''}`}>
                        <input
                          ...
                          onChange={(e) => updateMasterScheduleCell(idx, day - 1, e.target.value)}
