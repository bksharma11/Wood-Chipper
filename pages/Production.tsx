...
const ProductionPage: React.FC<ProductionPageProps> = ({ employees }) => {
  ...
  const [supervisor, setSupervisor] = useState(employees[13]?.name || '');
  ...
  const BunkerBar = ({ label, percentage }: { label: string, percentage: number }) => {
    ...
    <div className="space-y-8 py-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-900/50 p-6 rounded-xl border border-slate-700">
        <div className="flex items-center space-x-4 w-full sm:w-auto">
          <label className="text-xs text-slate-500 orbitron">SUPERVISOR SIGN-OFF:</label>
          <select 
            value={supervisor} 
